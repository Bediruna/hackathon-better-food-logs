...
const PROFANITY_WORDS = [
  ...
  // Racist/ethnic slurs (use caution)
  "chink",
  "gook",
  "spic",
  "wetback",
  ...
  // Self-harm/suicide (optional, for sensitive content screening)
  "suicide",
  "cutting",
  "selfharm",
  "depressed",
...
export const validateFoodEntry = (
  ...
  if (data.brand_name && data.brand_name.trim().length > 0) {
    if (data.brand_name.trim().length > 50) {
      ...
      !/^[a-zA-Z0-9\s\-\(\)\&\.\,\'\"]+$/.test(data.brand_name.trim())
    ) {
  ...
  if (
    ...
    data.serving_description.trim().length === 0
  ) {
    ...
    !/^[a-zA-Z0-9\s\-\(\)\&\.\,\'\"\/]+$/.test(data.serving_description.trim())
  ) {
  ...
  // Calories validation
  const parsedCalories = Number(data.calories);
