  ...
  try {
    ...
    if (localFoods.length > 0) {
      ...
      const existingFoodSignatures = new Set(
        existingFoods?.map(
          (f) =>
            `${f.name.toLowerCase().trim()}-${(f.brand_name || "")
              .toLowerCase()
              .trim()}-${f.serving_description.toLowerCase().trim()}-${
              f.serving_mass_g || 0
            }-${f.serving_volume_ml || 0}`
        ) || []
      );

      // Filter out foods that already exist in Supabase
      const newFoods = localFoods.filter((food) => {
        const signature = `${food.name.toLowerCase().trim()}-${(
          food.brand_name || ""
        )
          .toLowerCase()
          .trim()}-${food.serving_description.toLowerCase().trim()}-${
          food.serving_mass_g || 0
        }-${food.serving_volume_ml || 0}`;
      ...
      if (newFoods.length > 0) {
        ...
        const foodsToInsert = newFoods.map((food) => ({
          ...food,
        ...
        console.log(
          `Successfully synced ${insertedFoods?.length || 0} foods to Supabase`
        );
      }
    }
    ...
    if (localLogs.length > 0) {
      ...
      if (logFetchError) {
        console.error(
          "Error fetching existing food logs:",
      ...
      const existingLogSignatures = new Set(
        existingLogs?.map(
          (log) =>
            `${log.food_id}-${new Date(log.consumed_date).getTime()}-${
      ...
      const logsToInsert = localLogs
        .map(
          (
            ...
            if (!localFood) {
              console.warn(
                `Local food not found for log with food_id: ${log.food_id}`
              );
              return null;
            }

            // Create signature to find Supabase food ID
            const foodSignature = `${localFood.name.toLowerCase().trim()}-${(
              localFood.brand_name || ""
            )
              .toLowerCase()
              .trim()}-${localFood.serving_description.toLowerCase().trim()}-${
              localFood.serving_mass_g || 0
            }-${localFood.serving_volume_ml || 0}`;
            const supabaseFoodId = foodIdMap.get(foodSignature);

            if (!supabaseFoodId) {
              console.warn(
                `Supabase food ID not found for signature: ${foodSignature}`
              );
              return null;
            }
      ...
      if (logsToInsert.length > 0) {
        ...
        console.log(
          `Successfully synced ${logsToInsert.length} food logs to Supabase ${insertedLogs}`
        );
      }
    }
    ...
    console.log(
      "✅ Successfully synced all local data to Supabase and cleared local storage"
    );
  } catch (error) {
