...
export default function CreateFoodPage() {
  ...
  const checkDuplicate = async (foodData: FoodValidationData) => {
    ...
    let existingFoods: Array<{ name: string; brand_name?: string; serving_description: string }> = [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const validationData: FoodValidationData = {
        ...
        serving_mass_g: formData.serving_mass_g ? parseFloat(formData.serving_mass_g) : undefined,
        serving_volume_ml: formData.serving_volume_ml ? parseFloat(formData.serving_volume_ml) : undefined,
        ...
        sodium_mg: formData.sodium_mg ? parseFloat(formData.sodium_mg) : undefined,
        cholesterol_mg: formData.cholesterol_mg ? parseFloat(formData.cholesterol_mg) : undefined,
      };
      ...
      if (isDuplicateFood) {
        setIsDuplicate(true);
        setValidationErrors(['This food item already exists with similar details']);
        return;
      }
      ...
      const newFood: Omit<Food, "id"> = {
        ...
        brand_name: formData.brand_name ? sanitizeInput(formData.brand_name) : undefined,
        serving_description: sanitizeInput(formData.serving_description),
        serving_mass_g: formData.serving_mass_g ? formatNutritionValue(formData.serving_mass_g) : null,
        serving_volume_ml: formData.serving_volume_ml ? formatNutritionValue(formData.serving_volume_ml) : null,
  ...
  const isValid = formData.name.trim() && formData.serving_description.trim() && formData.calories.trim();
      ...
      {isDuplicate && (
        <div className="p-4 border border-yellow-200 rounded-lg bg-yellow-50">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="font-medium text-yellow-800">Possible Duplicate</h3>
              <p className="mt-1 text-sm text-yellow-700">
                A similar food item already exists. Please check if you meant to select an existing item instead.
              </p>
            </div>
          </div>
        </div>
